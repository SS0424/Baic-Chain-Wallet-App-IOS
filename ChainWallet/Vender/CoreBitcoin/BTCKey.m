...
- (ECDSA_SIG*)baicSignForHash:(NSData*)hash{
    CHECK_IF_CLEARED;
...
//        canonical &= ((signaturebytes[0] & 0x80) == 0);
//        canonical &= (!(signaturebytes[0] == 0 && ((signaturebytes[1] & 0x80) == 0)));
//        canonical &= ((signaturebytes[32] & 0x80) == 0);
//        canonical &= (!(signaturebytes[32] == 0 && ((signaturebytes[33] & 0x80) == 0)));
        nonce++;
    }
    return sig;
...
- (NSData*)compactSignatureFƒorHash:(NSData*)hash {
    CHECK_IF_CLEARED;
...
- (NSString*)myPublicKey {
    ...
    [pub appendData:self.compressedPublicKey];
    [pub appendData:[((NSData *)BTCRIPEMD160(self.compressedPublicKey)) subdataWithRange:NSMakeRange(0, 4)]];
...
- (NSString *)mBAICSignatureForHash:(NSData *)hash{
...
- (NSData*)eosCompactSignatureForHash:(NSData*)hash {
    ...
    if (nBitsR <= 256 && nBitsS <= 256) {
        ...
        for (int i=0; i < 4; i++) {
            ...
            if (ECDSA_SIG_recover_key_GFp(key2->_key, sig, hashbytes, hashlength, i, 1) == 1) {
                NSData *pubkey2 = [key2 compressedPublicKey];
                NSLog(@"1:%@\n2:%@",BTCBase58StringWithData(pubkey),BTCBase58StringWithData(pubkey2));
                // if ([[key2 myPublicKey] isEqualToString:[self myPublicKey]]) {
                
                if ([pubkey isEqual:pubkey2]) {
                    NSLog(@"111:%@",[self myPublicKey]);
                    NSLog(@"222:%@",[key2 myPublicKey]);
...
- (NSData*)baicCompactSignatureForHash:(NSData*)hash {
    CHECK_IF_CLEARED;
    NSMutableData* sigdata = [NSMutableData dataWithLength:65];
    unsigned char* sigbytes = sigdata.mutableBytes;
    const unsigned char* hashbytes = hash.bytes;
    ...
    ECDSA_SIG *sig = [self baicSignForHash:hash];
    ...
    [d appendData:sigdata];
    NSData *kd = [@"K1" dataUsingEncoding:NSASCIIStringEncoding];
    [d appendData:kd];
    NSData *checkSum = [((NSData *)BTCRIPEMD160(d)) subdataWithRange:NSMakeRange(0, 4)];
    
    
    NSMutableData *ret = [NSMutableData new];
    [ret appendData:sigdata];
    [ret appendData:checkSum];
